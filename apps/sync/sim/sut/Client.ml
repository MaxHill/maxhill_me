...
let rec spawn ~sw ~env ~cmd : t =
  ...
  Fiber.fork ~sw (fun () ->
      ...
      match status with
      | `Exited 0 -> traceln " TS process closed (exit 0)"
      | `Exited code ->
          failwith (Printf.sprintf "TS process exited unexpectedly (code %d)" code)
      | `Signaled signal ->
          failwith
            (Printf.sprintf "TS process terminated by signal %d" signal));
...
and handle_stderr stderr_r () =
  ...
  try
    while true do
      traceln " TS STDERR: %s" (Eio.Buf_read.line buf)
    done
  with End_of_file -> ()
...
and handle_stdout stdout_r requests () =
  ...
  with
  | End_of_file -> traceln " TS STDOUT: closed"
  | Yojson.Json_error msg as exn ->
      traceln " json parse error: %s" msg;
      raise exn
  | Ppx_yojson_conv_lib.Yojson_conv.Of_yojson_error (exn, _json) ->
      traceln " decode error: %s" (Printexc.to_string exn);
      raise exn
  | exn ->
      traceln " other error: %s" (Printexc.to_string exn);
      raise exn
