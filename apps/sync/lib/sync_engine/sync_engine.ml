...
let process_sync_request request =
  let* () = Sync_engine_validation.ensure_request_hash_valid request in
  let response =
    {
      base_server_version = request.last_seen_server_version;
      latest_server_version = request.last_seen_server_version;
      operations = [];
      synced_operations = [];
      response_hash = "";
    }
  in
  Ok { response with response_hash = hash_sync_response response }
...
let process_sync_request_with_connection connection request =
  ...
  try
    Repository.with_transaction connection (fun conn ->
        ...
        Ok
          (build_sync_response request ~inserted_versions ~unseen_operations
             ~unseen_rows))
  with exn ->
    Error (Storage_error ("transaction failed: " ^ Printexc.to_string exn))
