...
export class AuthClient {
  ...
  async authorize(): Promise<void> {
    const openAuth = await this.getOpenAuth();
    const result = await openAuth.authorize(REDIRECT_URI, "code", {
      pkce: true,
    });
  ...
  async handleCallback(): Promise<void> {
    ...
    const openAuth = await this.getOpenAuth();
    const exchanged = await openAuth.exchange(code, REDIRECT_URI, challenge.verifier);
    if (exchanged.err) {
  ...
  async getToken(): Promise<string | null> {
    ...
    try {
      const openAuth = await this.getOpenAuth();
      const refreshed = await openAuth.refresh(refresh, { access });
      if (refreshed.err) {
        await this.setUnauthenticated();
        return null;
      }
  ...
  async getCurrentUserID(): Promise<string | null> {
    ...
    try {
      const openAuth = await this.getOpenAuth();
      const verified = await openAuth.verify(
        subjects,
        access,
  ...
  private async getOpenAuth(): Promise<OpenAuthClient> {
  ...
  private async setUnauthenticated(): Promise<void> {
    await this.dbClear();
...
function extractUserID(subject: unknown): string | null {
  ...
  return typeof properties.userID === "string" ? properties.userID : null;
}
