...
type PuttDistance = 8 | 10 | 12 | 14 | 18 | 22;
type PuttOutcome = "holed" | "0-0.5m" | "0.5-1m" | "1-2m" | "2-3m" | "+3m";
type PuttResult =
  | { outcome: "holed" }
...
export class LagPuttingGameService {
  ...
  async recordPuttResult(
    ...
    if (game.putts[puttIndex].result !== null) {
      throw new Error(`Putt ${puttIndex} already recorded`);
    }
