...
pub fn bytes(frng: FRNG, size: usize) Error![size]u8 {
    if (frng.entropy < size) return error.OutOfEntropy;
...
pub fn range_inclusive(frng: *FRNG, Int: type, min: Int, max: Int) Error!Int {
    comptime (@typeInfo(Int).int.signedness == .unsigned);
    assert(min <= max);
...
pub const Driver = struct {
    ...
    pub fn main(
        ...
        io: std.Io,
        sut: []const u8,
        operation: union(enum) { replay: struct { size: u32, seed: u64 }, search: struct {
            attempts: u32 = 100,
    ...
    fn run_once(driver: Driver, options: struct { size: u32, seed: u64, quiet: bool }) !enum { pass, fail } {
        assert(options.size <= driver.buffer.len);
