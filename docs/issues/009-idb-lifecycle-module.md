...
Extract database lifecycle logic (open, close, version upgrades, object-store creation, index migration) from `IDBRepository` into a new `packages/syncdb/src/indexeddb/lifecycle.ts` module. This module owns the `IDBDatabase` handle and exposes a method to create transactions. Co-locate the store-name constants (`ROWS_STORE`, `OPERATIONS_STORE`, `CLIENT_STATE_STORE`) and index constants here since lifecycle is responsible for schema setup.

Create `packages/syncdb/src/indexeddb/index.ts` as the barrel file for the new directory.
