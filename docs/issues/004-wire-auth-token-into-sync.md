...
Connect the golf auth client to the syncdb sync layer so that sync requests include the bearer token, and 401 responses trigger token refresh with retry.

In the golf app's `db.ts` (or wherever the database is built), use the new `getHeaders` hook from syncdb to call the auth client's `getToken()` and return an `Authorization` header.
...
- docs/issues/001-syncdb-get-headers-hook.md
