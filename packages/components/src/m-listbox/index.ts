...
import {
    MListboxChangeEvent,
    MListboxFocusChangeEvent,
    MListboxSelectEvent,
    MListboxUnselectedEvent,
} from "./events";
...
} from "../utils/option-list-manager";
 ...
 * @attr {string} mode - Selection mode: "single-select" | "single-focus" | "multiple" (default: "single-select")
 ...
 * @prop {string} mode - Selection mode: "single-select" | "single-focus" | "multiple"
 ...
 *
 * <!-- Multiple selection -->
 * <m-listbox name="colors" label="Choose colors" mode="multiple">
...
export class MListbox extends MFormAssociatedElement {
    ...
    private render() {
        this.shadowRoot!.innerHTML = `
            ...
            <div part="input-wrapper" class="input-wrapper"><slot></slot></div>
        `;
    }
    ...
    private createOptionListManager(): OptionListManager {
        ...
        const selector = this.skip ? `${baseSelector}:not(${this.skip})` : baseSelector;

        // Map mode attribute directly to SelectionMode
        const selectionMode: SelectionMode = this.mode;
    ...
    connectedCallback(): void {
        ...
        if (valueAttr) {
            ...
            if (selectedOptions.length > 0) {
                ...
                this.value = this.mode === "multiple" ? selectedValues : selectedValues[0];
            }
        }
    }
