...
The combobox shows a clear button by default when the input has a value. The clear button allows users to quickly reset their selection.

- Enabled by default (`clearable="true"`)
- The clear button uses `tabindex="-1"` so it's not tabbable
- In multiple selection mode, the clear button only clears the search input, not the selected items
- The `m-input-clear` event fires before clearing and can be prevented
- To disable the clear button, set `clearable="false"`
...
<CodeExample html={`<div class="form">
  ...
  <div class="box" data-variant="surface" data-span="2" style="padding: var(--size-2);">
    ...
    <ul style="margin: 0.5rem 0 0 0; padding-left: 1.5rem;">
      <li>↑/↓ or Ctrl+P/N - Navigate (opens list)</li>
...
- `after` — content after the input (e.g., search icons)
- default slot — `<m-option>` elements for the listbox

**Note**: The `before` slot is used internally for multi-select chips and is not available for custom content.
