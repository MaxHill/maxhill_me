...
export class Table {
  ...
  async setRow(rowKey: ValidKey, value: any): Promise<void> {
    ...
    const row = await this.idbRepository.getRow(tx, this.tableSchema.tableName, rowKey);

    const dot = await this.nextDot(tx);
    const op: CRDTOperation = {
      type: "setRow",
      table: this.tableSchema.tableName,
      rowKey,
      value,
      dot,
    };
  ...
  index(indexName: any): Index {
    return new Index(this.tableSchema.tableName, indexName, this.idbRepository);
  }
