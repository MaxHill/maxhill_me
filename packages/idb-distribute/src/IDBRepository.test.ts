...
import { CRDTOperation, ORMapRow, TABLE_NAME, ROW_KEY } from "./crdt.ts";
...
describe("IDBRepository", () => {
    ...
    describe("Query operations", () => {
        it("should query rows using a single-field index with exact match", async () => {
            ...
            const indexDef = { name: "usersByAge", table: "users", keys: ["age"] };
            const range = IDBKeyRange.only(["users", 25]);

            const results: ORMapRow[] = [];
            for await (const row of repo.query(queryTx, indexDef, range)) {
                results.push(row);
            }
            queryTx.commit();
