import { openDB, type DBSchema, type IDBPDatabase, type IDBPObjectStore, type IDBPTransaction, type OpenDBCallbacks, type StoreNames } from 'idb';
import { WAL, type WALEntry } from './wal.ts';
...
export const dbQueries = {
    async getLastSyncedVersionTx(
        tx: IDBPTransaction<InternalDbSchema , ["_lastSyncedVersion", ...any[]], "readwrite" | "readonly">
    ...
    async getLastAppliedVersionTx(
        tx: IDBPTransaction<InternalDbSchema, ["_lastAppliedVersion", ...any[]], "readwrite" | "readonly">
    ): Promise<number> {
    ...
    async getLastSeenServerVersion(
        ...
        const store = tx.objectStore("_wal") as IDBPObjectStore<InternalDbSchema, ["_wal"], "_wal", "readwrite" | "readonly">;
                    ...
                    if (prop === 'createObjectStore') {
                        return (name: string, options?: IDBObjectStoreParameters) => {
                            if (options?.autoIncrement) {
                                throw new Error("Autoincrementing IDs are not supported in distributed systems as they cannot guarantee uniqueness across clients.");
                            }
