    ...
    async saveRow(
        ...
        if (Object.keys(row.fields).length > 0 || row.tombstone) {
            ...
            await promisifyIDBRequest(store.delete([
                row[TABLE_NAME],
                row[ROW_KEY] as IDBValidKey, /*Casting is fine here since ValidKey is a subset of IDBValidKey*/
            ]));
        }
    }

    query(tx: IDBTransaction, indexDefinition: IndexDefinition, range: IDBKeyRange): AsyncIterableIterator<ORMapRow> {
        validateTransactionStores(tx, [ROWS_STORE]);
        const store = tx.objectStore(ROWS_STORE);

        const [indexName, _] = indexDefinitionToIDBIndex(indexDefinition);
